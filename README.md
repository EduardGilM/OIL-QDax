...
This repository is a **fork of the original [QDax repository](https://github.com/adaptive-intelligent-robotics/QDax)** that implements two additional diversity metrics:

- **Lempel-Ziv (LZ)**
- **O-Information (OI)**

These metrics are exposed through a post-hoc archive descriptor (`compute_oil_descriptor`) that can be paired with any QD emitter in QDax. The `scripts/` pipeline below demonstrates them with MAP-Elites, PGA-ME and DCRL-ME, compared against the standard feet-contact descriptor.

## 🚀 Running the Pipeline

The LZ and O-Information metrics are used as a **post-hoc archive descriptor**: each rollout's observation sequence is summarized into a 2D behavior descriptor `(normalized LZ76 complexity, normalized O-Information)` that fills the MAP-Elites archive. This is compared against the standard QDax feet-contact descriptor.

All entry points live in `scripts/` and are run from the repo root as modules:

### 1. Train with OIL descriptors

```bash
python -m scripts.run_oil_posthoc [env_name] [emitter] [num_iterations] [--seed SEED]
```

- `env_name` — a `*_uni` locomotion env (default `halfcheetah_uni`): `ant_uni`, `humanoid_uni`, `halfcheetah_uni`, `hopper_uni`, `walker2d_uni`.
- `emitter` — `mapelites` | `pga` | `dcrlme` (default `dcrlme`).
- `num_iterations` — default `1000`.
- `--seed` — default `42`, or `$OIL_SEED` if set.

Output → `repertoires/oil_posthoc/<env>/<emitter>/<timestamp>/`: the saved repertoire, `metrics.npz`, `behavior_descriptors.npy`, `summary.json`, and a `figures/` folder with the metrics and archive maps.

### 2. Train the feet-contact baseline

Same training, but with the standard feet-contact behavior descriptor (requires a `*_uni` env):

```bash
python -m scripts.run_footcontact [env_name] [emitter] [num_iterations] [--seed SEED]
```

Output → `repertoires/foot_contact/<env>/<emitter>/<timestamp>/`.

### 3. Compare descriptor maps

Once an env has all three emitters under **both** `repertoires/oil_posthoc/` and `repertoires/foot_contact/`, generate the side-by-side comparison:

```bash
python -m scripts.plot_descriptor_comparison_maps
```

Output → `repertoires/comparison_maps/<env>_footcontact_vs_oil.png`.

### Example: full comparison on HalfCheetah

```bash
for emitter in mapelites pga dcrlme; do
  python -m scripts.run_oil_posthoc  halfcheetah_uni "$emitter" 1000
  python -m scripts.run_footcontact  halfcheetah_uni "$emitter" 1000
done
python -m scripts.plot_descriptor_comparison_maps
```
