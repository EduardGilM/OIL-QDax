...
def run_dcrlme_oil_test(env_name: str = "ant_oil", num_iterations: int = 10) -> None:
...
def test_dcrlme_oil(env_name: str) -> None:
    """Test function for pytest."""
    repertoire = run_dcrlme_oil_test(env_name, num_iterations=5)
...
if __name__ == "__main__":
    ...
    repertoire = run_dcrlme_oil_test("ant_oil", num_iterations=1000)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    repertoire_path = f"./repertoires/dcrlm_oil/{timestamp}/"
    os.makedirs(repertoire_path, exist_ok=True)
    repertoire.save(path=repertoire_path)
