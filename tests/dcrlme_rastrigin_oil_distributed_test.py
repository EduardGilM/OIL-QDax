"""
Distributed DCRL-ME test with Rastrigin OI/LZ76 wrapper for multi-GPU training.

This test demonstrates how to use DistributedMAPElites to parallelize
O-Information (OI) calculations across multiple GPUs for the Rastrigin environment.
"""
...
def run_dcrlme_rastrigin_oil_distributed(
    num_iterations: int = 10, num_gpus: int = None
) -> None:
    """
    Run DCRLME test with RastriginEnv and LZ76 wrapper using distributed training.
    ...
    # because rastrigin function naturally gives negative rewards that should not be clipped
    ...
    worst_fitness = episode_length * rastrigin_scoring(
        jnp.ones(num_dimensions) * maxval
    )
    ...
    print("Initializing distributed MAP-Elites for Rastrigin...")
    ...
    print(f"Running DCRL-ME with Rastrigin OI wrapper on {num_devices} GPUs...")
    ...
    fig1.savefig(
        os.path.join(plots_dir, f"dcrlm_rastrigin_distributed_metrics_{timestamp}.png")
    )
    plt.close(fig1)
    ...
    fig2.savefig(
        os.path.join(plots_dir, f"dcrlm_rastrigin_distributed_archive_{timestamp}.png")
    )
    plt.close(fig2)
...
def test_dcrlme_rastrigin_oil_distributed(num_iterations: int, num_gpus: int) -> None:
    """Test function for pytest with distributed training."""
    repertoire = run_dcrlme_rastrigin_oil_distributed(
        num_iterations=100, num_gpus=num_gpus
    )
    assert repertoire is not None

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    repertoire_path = f"./repertoires/dcrlm_rastrigin_oil_distributed/{timestamp}/"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run DCRL-ME with Rastrigin OI on multiple GPUs"
    )
    parser.add_argument(
        "--iterations",
        type=int,
    ...
    print(f"\n{'=' * 60}")
    print(f"Starting DCRL-ME with Rastrigin O-Information Distributed Training")
