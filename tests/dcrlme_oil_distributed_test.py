"""
Distributed DCRL-ME test with OI/LZ76 wrapper for multi-GPU training.

This test demonstrates how to use DistributedMAPElites to parallelize
O-Information (OI) calculations across multiple GPUs.
"""
...
def run_dcrlme_oil_distributed(
    env_name: str = "ant_oil", num_iterations: int = 10, num_gpus: int = 4
) -> None:
    """
    Run DCRLME test with distributed training across multiple GPUs.
    ...
    # Total batch size is divided across GPUs
    ...
    # Initialize params - no device dimension needed
    ...
    print("Initializing distributed MAP-Elites...")
    ...
    print(f"Running DCRL-ME with OI wrapper on {num_devices} GPUs...")
    ...
    fig1.savefig(os.path.join(plots_dir, f"dcrlm_distributed_metrics_{timestamp}.png"))
    plt.close(fig1)
    ...
    fig2.savefig(os.path.join(plots_dir, f"dcrlm_distributed_archive_{timestamp}.png"))
    plt.close(fig2)
...
@pytest.mark.parametrize(
    "env_name,num_iterations,num_gpus",
    [
        ("ant_oil", 5, 1),  # Single GPU test
        ("ant_oil", 5, 2),  # 2 GPUs test
    ],
)
def test_dcrlme_oil_distributed(
    env_name: str, num_iterations: int, num_gpus: int
) -> None:
    """Test function for pytest with distributed training."""
    repertoire = run_dcrlme_oil_distributed(
        env_name=env_name, num_iterations=num_iterations, num_gpus=num_gpus
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run DCRL-ME with OI on multiple GPUs")
    parser.add_argument(
        "--env", type=str, default="ant_oil", help="Environment name (default: ant_oil)"
    )
    parser.add_argument(
        "--iterations",
        type=int,
    ...
    print(f"\n{'=' * 60}")
    print(f"Starting DCRL-ME with O-Information Distributed Training")
    ...
    repertoire = run_dcrlme_oil_distributed(
        env_name=args.env, num_iterations=args.iterations, num_gpus=args.gpus
    )
