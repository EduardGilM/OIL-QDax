...
def run_dcrlme_sphere_oil_test(num_iterations: int = 10) -> None:
...
def test_dcrlme_sphere_oil(num_iterations: int) -> None:
    """Test function for pytest."""
    repertoire = run_dcrlme_sphere_oil_test(num_iterations=50)
...
if __name__ == "__main__":
    ...
    repertoire = run_dcrlme_sphere_oil_test(num_iterations=1)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    repertoire_path = f"./repertoires/dcrlm_sphere_oil/{timestamp}/"
    os.makedirs(repertoire_path, exist_ok=True)
    repertoire.save(path=repertoire_path)
